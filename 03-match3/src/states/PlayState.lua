...
function PlayState:enter(params)
    ...
    self.scoreGoal = 5000 + self.level * 500
end
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        -- add extra one second for each match
        self.timer = self.timer + 1

        -- add score for each match
        -- add score multiplier for better variety
        -- cross ×1.2, circle ×1.4, ..., star ×2
        for _, match in pairs(matches) do
            for _, tile in pairs(match) do
                self.score = self.score + 50 * (0.8 + 0.2 * tile.variety)
            end
        end
